...
import { computeCareerStats, computeSkillProfile, getGameRank } from "../logic/analytics";
import { formatTime } from "../logic/metrics";
import { getPersonalBests, loadGameHistory } from "../logic/persistence";
...
function StatCell({ label, value, best }: { label: string; value: string; best?: boolean }) {
  return (
    <div className="rounded-lg bg-gray-800 p-3 text-center group relative" title={STAT_TIPS[label]}>
...
export default function GameOverScreen({
  ...
  const history = useMemo(() => loadGameHistory(), []);
  ...
  return (
    <div className="mx-auto max-w-2xl py-8">
      <div className="mb-8 text-center">
        <p className="text-sm uppercase tracking-widest text-gray-500">Game Complete</p>
        <p className="mt-2 text-6xl font-extrabold text-white">{record.rating}</p>
        <p className="text-lg text-gray-400">Rating</p>
        {isBest("bestRating", record.rating) && (
          <p className="mt-1 text-sm font-semibold text-yellow-400">New Personal Best!</p>
        )}
        {rank !== null && history.length > 1 && (
          <p className="mt-1 text-sm text-gray-500">
            #{rank} of {history.length} games by rating
          </p>
        )}
      </div>
        ...
        <div className="mb-8">
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
            {comparisons.map((c) => {
              ...
              return (
                <div key={c.label} className="rounded-lg bg-gray-800 p-3 text-center">
      ...
      <div className="flex justify-center gap-4">
        ...
        <button
          ...
          className="rounded-xl bg-gray-700 px-8 py-3 font-semibold text-white transition hover:bg-gray-600"
        >
          High Scores
        </button>
      </div>
