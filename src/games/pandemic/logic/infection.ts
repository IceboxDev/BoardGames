...
export function infectCity(
  ...
  s = resolveOutbreak(s, cityId, color, new Set([cityId]));
  return s;
}

export function resolveOutbreak(
  state: GameState,
  cityId: string,
  color: DiseaseColor,
  chainSet: Set<string>,
  ...
  for (const neighbor of neighbors) {
    ...
    if (s.cityCubes[neighbor][color] >= MAX_CUBES_PER_CITY_COLOR) {
      ...
      s = resolveOutbreak(s, neighbor, color, chainSet);
    } else {
...
export function checkEradication(state: GameState, color: DiseaseColor): GameState {
  ...
  if (totalCubes === 0) {
    const s = { ...state, diseaseStatus: { ...state.diseaseStatus } };
