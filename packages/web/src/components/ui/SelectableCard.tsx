...
import { OPTION_ROW_BASE, OPTION_ROW_GHOST, OPTION_ROW_SELECTED } from "./option-row-chrome";
import { RADIUS_CARD_2XL, RADIUS_CARD_LG, RADIUS_CARD_XL } from "./radii";
...
//   row    — a dense single-pick LIST row (thumbnail + title inside a
//            scrolling picker). Flat until hovered, the shared option-row
//            accent pair when selected — the same pair CheckRow draws, so a
//            picked row and a checked row read identically.
//
...
export function SelectableCard({
  ...
  const body =
    ...
    (orientation === "horizontal" ? (
      <>
        {icon && (
          <div
            className={`flex h-9 w-9 shrink-0 items-center justify-center ${RADIUS_CARD_LG} bg-surface-700/40 text-fg-secondary transition-colors ${TILE_ICON_HOVER[tone]}`}
          >
            {icon}
          </div>
        )}
        <div className="min-w-0 flex-1">
          {title && (
            <div className="text-sm font-medium text-fg-secondary transition-colors group-hover:text-fg-strong">
              {title}
            </div>
          )}
      ...
      <>
        {icon && (
          <div
            className={`flex h-14 w-14 items-center justify-center ${RADIUS_CARD_2XL} transition-colors duration-300 ${TILE_ICON[tone]}`}
          >
            {icon}
          </div>
        )}
        <div>
          {title && (
            <div
              className={`text-base font-semibold text-fg-strong transition-colors ${TILE_TITLE_HOVER[tone]}`}
            >
              {title}
            </div>
          )}
  ...
  if (variant === "row") {
    cls = cn(
      OPTION_ROW_BASE,
      "w-full",
      padding === "sm" ? "gap-2.5 px-2 py-1.5" : "gap-3 px-3 py-2",
      isToggle && selected ? OPTION_ROW_SELECTED : OPTION_ROW_GHOST,
      sharedTail,
    );
    ...
    cls = cn(
      ...
      isToggle && selected
        ? "border-line-strong bg-surface-800/80 shadow-lg"
    ...
    cls = cn(
      ...
      isToggle && selected ? TILE_SELECTED[tone] : `border-line ${TILE_HOVER_BORDER[tone]}`,
      sharedTail,
    );
  } else {
    cls = cn(
      ...
      isToggle && selected
        ? TILE_SELECTED[tone]
        : `border-line ${TILE_HOVER_BORDER[tone]} ${TILE_HOVER_GLOW[tone]}`,
      sharedTail,
    );
  }
