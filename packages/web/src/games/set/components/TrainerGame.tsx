...
interface TrainerGameProps {
  onViewHistory?: () => void;
}

export default function TrainerGame({ onViewHistory }: TrainerGameProps) {
  ...
  const { history, save } = useSetHistory();
  ...
  // Auto-start when entering trainer mode
  useEffect(() => {
    ...
    return (
      <div className="mx-auto max-w-3xl px-6">
        <GameOverScreen
          ...
          onViewHighScores={onViewHistory ?? (() => {})}
        />
      </div>
    );
  }
