...
import { getLegalActions, getValidCombos } from "@boardgames/core/games/exploding-kittens/rules";
import type {
  Action,
  Card as CardData,
  GameState,
...
export default function PlayerHand({ state, onAction, disabled }: PlayerHandProps) {
  ...
  const player = state.players[humanIndex >= 0 ? humanIndex : state.currentPlayerIndex];
    ...
    <div>
      ...
      {hand.length === 0 ? (
        ...
        <CardFan
          ...
          onCardClick={(card) => toggleCard(card)}
          disabled={disabled}
        />
      )}
    </div>
