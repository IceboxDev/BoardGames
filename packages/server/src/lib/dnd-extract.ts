...
function buildTurnPrompt(ctx: CombatTurnContext): string {
  ...
  lines.push(
    "1. LEGALITY: alert ONLY on clear violations — action economy (one action, one bonus action per turn), spell slots or ammunition the notes/history show as already spent, conditions that forbid the act, abilities the character demonstrably does not have, or ranges that contradict explicitly tracked positions. On abilities, judge direction carefully: each PC's listed combat options are a SUMMARY — a floor, never a ceiling. Whatever is listed they definitely have; but the DM is reading the character's FULL physical sheet, which contains weapon riders and masteries (e.g. Cleave), feats, and item properties the summary omits. When the DM invokes a named feature that is not in the list, trust that it is printed on the sheet and adjudicate it as reported. Alert 'they do not have that' ONLY when the claim contradicts what IS known — a leveled spell from a character whose sheet shows no spellcasting at all, or a feature the notes/history show as already spent. The tracked state is a coarse sketch: positions are approximate free text and most battlefield facts are never recorded. The DM is looking at the real battlefield — when a precondition (advantage, an ally within 5 ft, line of sight, flanking) is merely NOT RECORDED, assume the DM ruled it correctly at the table and do not alert. Absence of evidence is not a violation. COMPLETENESS is part of legality: when the reported action has known mechanics — the character's verbatim attacks, their action cards, or a well-known spell — the report must resolve them fully. A hit that lands needs its damage stated; a save-or-effect spell needs the save outcome reported and, on a failure, its printed consequence applied (on a successful save apply exactly what the ability says — for many spells that is nothing). If a required consequence is left unstated (a landed hit with no damage, a cast spell with no save outcome, a forgotten rider), alert asking for the missing numbers rather than guessing or silently skipping them — but ONLY when the missing number would actually change the tracked state. When the omitted detail cannot matter (damage dice against a creature already at 0 hp where only crit-or-not decides anything, a rider with no valid target), the report is complete as given: the DM's stated conclusion about the consequence IS the resolution. Each PC's sheet numbers (speed, AC, HP, ability scores) are AUTHORITATIVE exactly as printed — never re-derive or 'correct' them from rulebook defaults; sheets at this table may follow different rule editions. When there are violations, return alerts naming each one, an empty narration, and NO updates.",
  );
  lines.push(
