...
export function applyForecastReorder(state: GameState, newOrder: number[]): GameState {
  ...
  s.phase = s.preForecastPhase ?? "actions";
  s.preForecastPhase = null;

  if (s.phase === "discard" && s.discardingPlayerIndex !== null) {
    const dp = s.players[s.discardingPlayerIndex];
    if (dp.hand.length <= HAND_LIMIT) {
