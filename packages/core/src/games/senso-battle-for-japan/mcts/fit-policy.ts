...
//        [--name POLICY_MODEL] [--note "fitted on ..."] [--players 3]
// `--players` keeps only the rows recorded at that table size (a per-table model).
...
for (const file of files) {
  for (const line of readFileSync(file, "utf8").split("\n")) {
    if (!line) continue;
    const row = JSON.parse(line) as Row & { n?: number };
...
const body = `${note ? `// ${note}\n` : ""}// Generated by fit-policy.ts on ${new Date().toISOString().slice(0, 10)} from ${rows.length} decisions${players > 0 ? ` (${players}-player tables only)` : ""}
