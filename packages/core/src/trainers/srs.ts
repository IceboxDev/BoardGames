// The trainers' spaced-repetition scheduler: SM-2, day-granular like Anki.
// Quiztopia self-grades with two grades ("again" / "good"); auto-graded
// trainers (geography) also use "hard" and "easy". Pure and shared by the server
...
export const SRS = {
  ...
  EASE_HARD: -0.15,
  EASE_EASY: 0.15,
  /** Interval growth of a "hard" review. */
  HARD_FACTOR: 1.2,
  /** Extra growth of an "easy" review, on top of the ease. */
  EASY_BONUS: 1.3,
  /** An "easy" new or learning card skips straight to this interval. */
  EASY_INTERVAL: 4,
  FIRST_INTERVAL: 1,
  GRADUATE_INTERVAL: 3,
  MAX_INTERVAL: 365,
...
export function applyReview(
  ...
  const reviewIn = (intervalDays: number, ease = prev.ease): SrsState => {
    const days = Math.min(SRS.MAX_INTERVAL, intervalDays);
    return {
      ...base,
      state: "review",
      intervalDays: days,
      ease: round2(Math.max(SRS.EASE_MIN, ease)),
      dueDate: addDays(at.localDate, days),
    };
  };
  if (grade === "easy") {
    if (prev.state === "review") {
      const grown = Math.round(prev.intervalDays * prev.ease * SRS.EASY_BONUS);
      return reviewIn(Math.max(prev.intervalDays + 1, grown), prev.ease + SRS.EASE_EASY);
    }
    if (prev.state === "relearning") return reviewIn(prev.intervalDays + 1);
    return reviewIn(SRS.EASY_INTERVAL);
  }
  if (grade === "hard") {
    switch (prev.state) {
      case "new":
      case "learning":
        // Correct, but shakily: another learning step tomorrow, no graduation.
        return {
          ...base,
          state: "learning",
      ...
      case "review": {
        const grown = Math.round(prev.intervalDays * SRS.HARD_FACTOR);
        return reviewIn(Math.max(prev.intervalDays + 1, grown), prev.ease + SRS.EASE_HARD);
      }
      case "relearning":
        return reviewIn(prev.intervalDays);
    }
  }
  if (grade === "good") {
    switch (prev.state) {
